...
There are several papers that we'd like to mention developing the topics related to the present work. Yoshua Bengio, Aaron Courville, and Pascal Vincent review representation learning and why it is importatn,
...
we refer the reader to \cite{vincent2010stacked}. Hinton et al. describe in \cite{hinton2006learning} how to successfully train deep multilayer networks, like the stacked autoencoder we use in this paper. The theory and methods behind the backpropogation algorithm that we use
is classic and can be found in \cite{hecht1989theory}. We are presently training the neurla network using a stochastic gradient descent algorithm for neural networks, which is a topic covered in \cite{bottou-91c}. 
Future plans include using a genetic algorithm to train the autoencoder instead of backpropogation. A survey of genetic algorithms can be found in \cite{srinivas1994genetic}.
...
As an example, we describe how to train a simple 3-layer neural network, then afterwards discuss how to specialize this for a n autoencoder. We start with a random weight matrix $W$ and random biases $b$ and $b^{'}$. We take a given input $x$, and feed it forward through the network and 
...
$s(x_j)$    & $1/(1+e^{-x_j})$                                              \\                                              
$b_j^{\{O,H\}}$ & Bias vector for output/hidden layer, respectively          \\                                              
$\eta$ & Learning rate                                                       \\
\end{tabular}
\end{table}
...
In order to ensure that updates and feedforward activation can be done in a reasonable amount of time we have parallelized the update step
(parallel for loops for updating the weight matrices and biases), as well as parallelizing the matrix multiplication, vector addixction,
sigmoid transfer function, and error computations. Each stochastic gradient descent step is taken sequentially at present, i.e. we train
the weights and biases with one training example (and therefore loss function value) at a time.
...
for a more general neural network. In an autoencoder, the decoding weight matrix is simply the transpose of the encoding weight matrix (so it maps the output back to the same space as the input).
For a stacked autoencoder we sandwich more hidden layers (with different weights and biases) between the input and the output layers, but we train them one at a time in a greedy fashion \cite{hinton2006learning}.
